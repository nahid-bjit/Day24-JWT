...
class ProductController {
        ...
        try {
            ...
            if (products.length > 0) {
                return res
                    ...
                    .send(success("Successfully received all users", { result: products, total: products.length }));
            }
    ...
    async getOneById(req, res) {
        try {
            ...
            if (product) {
                return res.status(HTTP_STATUS.OK).send(success("Successfully received the product", product));
            } else {
